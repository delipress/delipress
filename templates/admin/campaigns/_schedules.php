...
$pagedType  = $typeCampaign = "schedules";
$paged      = (isset($_GET["paged-" . $pagedType])) ?
                (int) $_GET["paged-" . $pagedType] :
                (
                    (isset($_GET["paged"])) ?  (int) $_GET["paged"] : 1
                );
$numberPerPage  = apply_filters(DELIPRESS_SLUG . "_campaigns_schedule_per_page", 10);
...
$campaigns = $this->campaignServices->getCampaignsWithProvider($provider["key"],
    array(
        "post_status" => "publish",
        "meta_query"  => array(
            "relation" => "AND",
...
$classList    = "delipress__checkbox__input__schedule";
$nameCheckbox = "campaignsSchedule[]";

if(!empty($campaigns)):
?>

<h2 class="delipress__opener delipress__opener--is-open js-delipress-opener"><?php esc_html_e('Scheduled campaigns', "delipress"); ?>  <span class="dashicons dashicons-arrow-down-alt2"></span></h2>
