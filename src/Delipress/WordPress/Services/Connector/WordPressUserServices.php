...
/**
 * WordPressUserServices
 *
 * @author Delipress
 * @version 1.0.0
 * @since 1.0.0
 */
class WordPressUserServices extends AbstractConnectorServices implements ContainerServiceInterface {
    ...
    public function prepareLists($listId){
        $range              = apply_filters(DELIPRESS_SLUG . "_range_wordpress_user_connector", $this->range);
        
        $totalUsers         = count_users();
        $countCall          = ceil($totalUsers["total_users"] /  $range);
        $prepareLists = array();
     ...
     * @return array
     */
    protected function getSubscribers(ListModel $list, $offset, $limit){
